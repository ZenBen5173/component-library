...
export default function SearchModalDemo() {
  ...
  return (
    <div className="flex min-h-[620px] items-start justify-center bg-neutral-100 p-8 pt-24 dark:bg-background">
      {/* The collapsed state — your own trigger. */}
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="flex w-full max-w-sm items-center gap-2 rounded-xl border border-black/10 bg-white px-3.5 py-2.5 text-sm text-muted-foreground shadow-sm transition-colors hover:border-black/20 dark:border-white/10 dark:bg-neutral-900 dark:hover:border-white/20"
      >
        <Search className="size-4" />
        <span className="flex-1 text-left">Search…</span>
        <kbd className="rounded border border-black/10 px-1.5 py-0.5 font-mono text-[10px] text-muted-foreground dark:border-white/15">
          ⌘K
        </kbd>
      </button>
